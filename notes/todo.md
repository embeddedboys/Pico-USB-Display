...
**第三次复现**（`touch_draw.py --mode grid`，同时用 gdb 把触摸值钉死）：这是**实际用法**
的场景 —— 每个触摸报告在面板上画一个小矩形（30 次/秒、每次几百字节）。板子又失联，
而且这次**没有自己恢复**（等了 5 分钟、重试十几次都没回来，需要断电重启）。Pico 没事
（SWD 连上去还能跑）。这次多了一个变量：gdb 每秒把 CPU 停 90 次（每次 `ft6236_*` 调用
都断点返回），**不排除是"在 USB 传输中途停 CPU"本身触发的**，所以第三次的归因存疑；
前两次与调试器无关。`touch_draw.py` 因此默认 `--rate 20 --gap-ms 2`。
