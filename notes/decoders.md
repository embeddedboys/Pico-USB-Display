...
> 这是**回调版**解码器的账。QOI 现在默认走**非回调 + band 乒乓**（下下节），批次乒乓对它
> 不再适用；`PUD_DECODER_PINGPONG` 仍然管着回落到回调版的那条路径和 RLE。
...
### QOI 解码 API：回调版 vs 非回调版（2026-09 实测）
...
- **B 证明"加大批次"没用**：回调版放到整个 band 一批（一次 flush）反而丢了"解码与刷屏
  重叠"，纯色从 3.85 涨到 6.10 ms。所以省不下时间不是批次太小的错。
...
- **D 两者兼得**：解码（非回调）写进 buffer A → 起异步 flush → 立刻返回；下一张解码进
  ...
  于是**解码与刷屏重新重叠**，且刷屏不再阻塞解码。
- **代价**：2 × `band_pixels` × 2 B = **87 KB**（64 KB 传输时）。原来那两块 8 行的
  `qoi_buf_a/b`（15 KB）在默认路径上不再需要（保留给回落路径）。
...
- **适用范围**：只对"矩形像素数 ≤ `(PUD_MAX_TRANSFER-16)/3`"的传输走这条路；更大的矩形自动
  回落到回调版。`-DQOI_NONCALLBACK=0` 可以强制用回调版做 A/B，`=1` 是"非回调但不乒乓"
  （只有测量价值，没有重叠会变慢）。**RLE 还是老的 8 行回调版**，同样的改动没做（见
  [todo.md](todo.md)）。

### USB 传输缓冲（`PUD_MAX_TRANSFER`）不背锅

链路实测 **1.11~1.13 MB/s**，从每帧 11 KB 到 444 KB 都是平的（= 全速理论上限 1.216 MB/s 的
93%），所以 CherryUSB 端口只给每个端点 64 B DPRAM 单缓冲**不是瓶颈**，band 多几个也不影响。

把 `PUD_MAX_TRANSFER` 从 64 KB 降到 32 KB：省 109 KB RAM，但设备侧多花 2~3%，而且**小载荷
反而明显变慢**（纯色 4.6 → 6.5 ms/帧，band 数 8 → 15）。128 KB 之前也量过是持平的。
**结论：RP2350 保持 64 KB、RP2040 保持 32 KB**（现在的定义就是按板子分的）。

### 那"存一整屏"能不能省掉 `set_addr_win`？（结论：不值得）

想法：RP2350 内存大，设备存一整屏（480×320×2 = **300 KB**）就可以不反复设绘制窗口。
实测和算账都不支持：

- 每次 flush（一次 `set_addr_win` + 起一次异步像素 DMA）实测 **22 µs**（照片档 `flush_us`
  3519 µs / 160 次），一帧 8 个 band 合计 **176 µs ≈ 设备侧 0.8%**，在端到端 86 ms 里是 **0.2%**。
- 省不掉**主机侧的 EP0 窗口请求**：那是按"传输"算的，实测 0.141 ms × 8 = 1.1 ms/帧 ≈ 1.3%。
- 设备**不知道一帧的多个 band 何时结束**（协议里每个传输都是一份独立的、自包含的矩形图像），
  所以即便有全屏缓冲也不能把 8 次 flush 合成 1 次 —— 除非协议加"帧边界"标记（那时省下的
  也只是上面那 0.8%）。
- 内存本来就不够：现在（非回调 + band 乒乓）**RAM 312336 B = 59.6%**，再加 300 KB 会到
  ~605 KB > 512 KB；即使砍掉乒乓缓冲（87 KB）和一块帧槽（64 KB），也还有 ~89% 占用，
  剩下几十 KB 要放堆和任务栈。**不做。**
...
逐字节一致（含下面的 run 批量填充优化）；QOI 同理（`rgb565-qoi` → `src/decoders/qoi/`）。
两个库现在各有一个**放置钩子**（`RGB565_RLE_SECTION` / `RGB565_QOI_SECTION`，默认空，
保持 freestanding C99 可移植），本项目用它把解码循环放进 SRAM
（`PUD_CODEC_IN_RAM`，设备侧快 2~9%，见 [architecture.md](architecture.md)）——
**要改行为就先改上游，再把文件整体拷回来**，别直接改 vendored 文件。
...
板上 A/B（**当时是 150 MHz** RP2350；现在是 225 MHz，见 [architecture.md](architecture.md)；
480×320 按 8 行分批，交替烧写、同一主机同一脚本、`--frames 100`；
...
压缩率差通过主机侧**分带**来补偿（每带 ≤ 21835 像素）。详见
